...
function stripAmount(value) {
	return AMOUNT.exec(normalize(value))?.[1];
}
...
function parseOfficialPricing(html) {
	...
	const windows = [[footnote[1], footnote[2]], [footnote[3], footnote[4]]];
	if (windows[0][0] !== "01:00" || windows[0][1] !== "04:00" || windows[1][0] !== "06:00" || windows[1][1] !== "10:00") return;
...
*
* Only this route fetches the fixed DeepSeek page; the client never talks to
...
function normalizeUsageSample(usage) {
	...
	} else if (cacheReadTokens === void 0 || cacheWriteTokens === void 0) return;
	return {
		inputTokens,
		outputTokens,
		...cacheReadTokens === void 0 ? {} : { cacheReadTokens },
